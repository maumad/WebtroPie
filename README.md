...
Default options can be found in WebtroPie/app/config/settings.cfg but please note that this file is overwritten after an update. Options can be changed in the web appication itself which writes changes to /home/pi/.webtropie/settings.cfg, values in this file overrides those in the WebtroPie/app/config/settings file and are not overwritten after an update of WebtroPie.
