...
Default options can be found in config/settings.cfg but please note that this file is overwritten after an update. Options can be changed in the web appication itself which writes changes to config/settings.cfg, values in the preferences file overrides those in the settings file and are not overwritten after an update of WebtroPie.
