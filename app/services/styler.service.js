...
(function() {
   ...
   function styler(config, util, $document)
   {
      ...
      function createCarouselStyle(carousel, logo)
      {
         // alredy done?
         ...
         //style['color'] = util.isLight(carousel.color) ? '#222' : '#888';
         style['color'] =  '#222';
         ...
         {
            self.logo_vw = carousel.size.w;
            //self.logo_w = 0.5 * 1;
            self.logo_vh = carousel.size.h / carousel.maxLogoCount;
            //self.logo_h = 0.5 * 1 / carousel.maxLogoCount;
         }
         else
         {
            self.logo_vw = carousel.size.w / carousel.maxLogoCount;
            //self.logo_w = 0.5 * 1 / carousel.maxLogoCount;
            self.logo_vh = carousel.size.h;
            //self.logo_h = 0.5 * 1;
         }
...
//console.log(style);
      ...
      function createViewStyles(view, keep_style)
      {
         ...
         if (view.name == 'system' && !keep_style)
         {
            ...
            createCarouselStyle(view.carousel.systemcarousel, view.image.logo);
            if (!view.text)
            {
                view.text = {};
            }
...
*/
      ...
      function setSystemLogoStyle(cell, pct)
      {
         ...
         if (self.carousel.type == 'vertical')
         {
             index = cell.index + pct / self.logo_vh;
             top   = pct
                     + self.carousel.size.h / 2       // half way vertically in carousel area
                     + cell.index * self.logo_vh;            // index position
         }
         else
         {
             index = cell.index + pct / self.logo_vw;
             left  = pct
                     + self.carousel.size.w / 2       // half way horizontally in carousel area
                     + cell.index * self.logo_vw;            // index position
         }
